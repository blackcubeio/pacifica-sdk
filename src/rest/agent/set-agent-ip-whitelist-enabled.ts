...
export function setAgentIpWhitelistEnabled(
  ...
  const request = buildSignedRequest(OperationType.SetAgentIpWhitelistEnabled, payload, label);
  return httpPost<null>('/agent/ip_whitelist/toggle', request, label).then(() => undefined);
}
