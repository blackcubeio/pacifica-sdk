...
export function addToBlacklist(params: VaultSymbolsParams, signer?: Signer): Promise<void> {
  const payload = { lake: params.lake, symbols: params.symbols };
  const request = buildSignedRequest(OperationType.AddLakeBlacklist, payload, signer);
